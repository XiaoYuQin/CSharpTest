...
namespace SDcard
{
    class SDUtils
    {
        ...
        private static extern SafeFileHandle CreateFileA(string lpFileName, uint dwDesiredAccess, uint dwShareMode, IntPtr lpSecurityAttributes
        ...
        public SDUtils(string DriverName)
        {
            try
            {
                ...
                GetSectorCount();
                Console.WriteLine("SDUtils init ok");
            }
            catch (Exception)
            {
                Console.WriteLine("SDUtils init error");
            }
        }
        int number1;
        /// <summary>
        ...
        public void GetSectorCount()
        {
            ...
            Console.WriteLine("ReturnByte[0] = " + ReturnByte[0]);
            Console.WriteLine("ReturnByte[1] = " + ReturnByte[1]);
            ...
            {                
                ...
                Console.WriteLine("DOS _SectorLength = " + _SectorLength);
            }
            if (ReturnByte[0] == 0xEB && ReturnByte[1] == 0x52)          //NTFS好象是64位
            {
                _SectorLength = BitConverter.ToInt64(new byte[] { ReturnByte[40], ReturnByte[41], ReturnByte[42], ReturnByte[43], ReturnByte[44], ReturnByte[45]
                    , ReturnByte[46], ReturnByte[47] }, 0);
                Console.WriteLine("NFS _SectorLength = " + _SectorLength);
            }
